...
function JobApplicationsPage() {
    ...
    const handleAccept = async () => {
        ...
        const response = await acceptApplication(jobApplicationsData[currentPage - 1].applicationId);
        if (response.success) {
            console.log(response.message);
            setSuccessResponse(response.message);
            jobApplicationsData[currentPage - 1].applicationStatus = 1;
            setFailureResponse("");
        } else {
            setSuccessResponse("");
    ...
    const handleReject = async () => {
        console.log(currentPage);
        const response = await rejectApplication(jobApplicationsData[currentPage - 1].applicationId);
        if (response.success) {
            setSuccessResponse(response.message);
            jobApplicationsData[currentPage - 1].applicationStatus = 2;
            setFailureResponse("");
            console.log(response.message);
        } else {
            setFailureResponse(response.message);
        ...
        const fetchJobApplications = async () => {
            try {
                ...
                if (response.length === 0) {
                    setNoApplicationsMessage("No applications available for this job.");
                } else {
                    setNoApplicationsMessage("");
                }
                setJobApplicationsData(response);
            } catch (error) {
                setNoApplicationsMessage("Error fetching job applications. Please try again later.");
            }
        };
    ...
    return (
        <div className="applications-container">
            {
                noApplicationsMessage ? <p className="no-applications-message">{noApplicationsMessage}</p> :
                    <>
